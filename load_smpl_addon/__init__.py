...
class OpenSmplPklOperator(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        # Create a text label with the filename
        filename = os.path.basename(self.filepath)
        bpy.ops.object.text_add(location=(0, 0, obj.dimensions.z + 0.2))  # Position text above the model
        text_obj = bpy.context.object
        text_obj.data.body = filename
        text_obj.parent = obj  # Parent the text to the SMPL model
        text_obj.data.size = 0.67  # Adjust text size
        text_obj.data.align_x = 'CENTER'
        text_obj.data.align_y = 'CENTER'

        # Rotate the text to face the correct direction
        text_obj.rotation_euler[0] = 0  # Adjust if necessary
        text_obj.rotation_euler[1] = math.pi  # Adjust if necessary
        text_obj.rotation_euler[2] = math.pi  # 180 degrees in radians to flip it
